  ...
  const columns_mobile = [
    {
      ...
      render: (v) => (
        <Link
          to={`/prosetting/${v.toLowerCase()}`}
          style={{ color: "#ff324c", fontWeight: "bold" }}
          key={v}
        >
          {v} <SearchOutlined style={{ color: "white" }} />{" "}
        </Link>
      ),
    },
    {
  ...
  const columns = [
    {
      ...
      render: (v) => (
        <Link
          to={`/prosetting/${v.toLowerCase()}`}
          style={{ color: "#ff324c", fontWeight: "bold", fontSize: "1.15rem" }}
          key={v}
        >
          {v} <SearchOutlined style={{ color: "white" }} />{" "}
        </Link>
    ...
    >
      ...
      >
        ...
        <MetaTags>
          ...
          <meta
            name="description"
            content={`발로란트 프로 혹은 스트리머들이 사용하는 마우스 감도, 키보드 설정, 그래픽 설정, 조준선 설정 등을 확인가능합니다!`}
          />
        </MetaTags>
        <div
          style={{
        ...
        >
          ...
          <div style={{ fontSize: "1.1rem" }}>
            CS:GO 프로 선수 및 스트리머들이 발로란트에서 평균적으로 가장 많이
            사용하는
          </div>
          <div style={{ fontSize: "1.1rem", marginLeft: "2rem" }}>
            DPI 는 <Bold>400</Bold> 입니다.{" "}
            <span style={{ fontSize: "0.8rem" }}>(2순위 800)</span>
          </div>
          <div style={{ fontSize: "1.1rem", marginLeft: "2rem" }}>
            평균 eDPI 는 <Bold>{avgEDpi}</Bold> 입니다.{" "}
            <span style={{ fontSize: "0.8rem" }}>(eDPI = dpi × 감도)</span>{" "}
          </div>
          <div style={{ fontSize: "1.1rem", marginLeft: "2rem" }}>
            조준경 감도 배율은 <Bold>1.00</Bold> 입니다.{" "}
            <span style={{ fontSize: "0.8rem" }}>(평균치 0.914)</span>
          </div>
          <div style={{ fontSize: "1.1rem", marginLeft: "2rem" }}>
            마우스 폴링 레이트는 <Bold>1000 hz</Bold> 입니다.
          </div>
          <div style={{ fontSize: "1.1rem", marginLeft: "2rem" }}>
            미니맵 설정은{" "}
            <Bold>
              회전:회전 / 플레이어 중앙에 고정:꺼짐 / 미니맵 크기:0.978 / 미니맵
              확대:0.9 / 미니맵 시야 범위 표시:켜짐
            </Bold>{" "}
            입니다.
          </div>
          <div style={{ fontSize: "1.1rem", marginLeft: "2rem" }}>
            조준선 설정은{" "}
            <Bold>
              색상:녹색 / 1 / 4 / 2 / 2 / 화면 중앙 도트: 꺼짐 / 오류: 꺼짐
            </Bold>{" "}
            입니다.
          </div>
        </div>
        ...
        <div style={{ backgroundColor: "#fafafa" }}>
          {window.innerWidth < 576 ? (
            <Table
              ...
              dataSource={dataSource.slice(0).sort(dynamicSort("name"))}
              style={{ width: "100%" }}
              pagination={false}
            />
          ) : (
