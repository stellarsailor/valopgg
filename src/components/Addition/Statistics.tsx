...
export default function Statistics() {
  ...
  useEffect(() => {
    axios.get(`${apiServer}/statistics`).then((res) => {
  ...
  const columns = [
    {
      ...
      render: (v) => (
        <Link
          to={`/agent/${agent[v].name.toLowerCase()}`}
          key={v}
          style={{
            display: "flex",
            flexDirection: "row",
        ...
        >
          <img
            src={`${staticServer}/agents/${agent[
              v
            ].name.toLowerCase()}-headshot.png`}
            style={{
              width: "3rem",
              borderRadius: "3rem",
              border: `3px solid ${agent[v].signature_color}`,
            }}
    ...
    <Row justify="center" style={{ backgroundColor: "rgba(19, 28, 46, 0.95)" }}>
      ...
      <Col xs={24} sm={22} md={20} lg={20} xl={15}>
        ...
        <MetaTags>
          ...
          <meta
            name="description"
            content={`발로란트의 요원들의 픽률, 승률, 킬 데스 통계 정보를 확인가능합니다.`}
          />
        </MetaTags>
        <Row
          style={{ width: "100%", height: "100%", backgroundColor: "#202b43" }}
          justify="center"
        >
          <div
            style={{ fontWeight: "bold", fontSize: "1.4rem", margin: "1.5rem" }}
          >
            요원별 통계(Beta)
          </div>
          <Table
            columns={columns}
            dataSource={agentStat}
            style={{ width: "100%" }}
            pagination={false}
            loading={statLoading}
          />
          <div style={{ margin: "1rem" }}>
            해당 데이터는 발로란트의 공식 데이터가 아니며, 발로란트 북미
            서버에서 수집된 경기를 종합한 데이터 입니다.
          </div>
        </Row>
      </Col>
      <SideAds />
