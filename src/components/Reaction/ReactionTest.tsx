...
import { getTier } from "../../logics/getTier"
...
export default function ReactionTest() {
  ...
  useEffect(() => {
    axios.get(`${apiServer}/reaction/today`).then((res) => {
  ...
  const arrAvg = (arr) => {
    ...
    const avg =
      (sum - Math.max(...filtered) - Math.min(...filtered)) /
      (filtered.length - 2)
    return avg
  }
  ...
  return (
    ...
    >
      ...
      <Col xs={24} sm={22} md={20} lg={20} xl={15}>
        ...
        <MetaTags>
          <title>
            발로그 발로란트 FPS 반응속도 측정 테스트 Valorant Reaction Test
          </title>
          <meta
            name="description"
            content={`발로란트 게임을 옮겨놓은듯한 시뮬레이터로 본인의 FPS 반응속도 재능을 측정해보세요! You can check your reaction time in Valorant!`}
          />
        </MetaTags>
        <Row justify="center">
          {todaysMinimumDesktop === 0 || todaysMinimumMobile === 0 ? null : (
            <Col span={24} style={{ width: "100%" }}>
              <div style={{ margin: 10, fontSize: "1rem" }}>
                <CrownFilled style={{ color: "gold", marginRight: 10 }} />
                오늘의 1등 :
                <DesktopOutlined
                  style={{
                    color: "lightgray",
                ...
                />{" "}
                <span style={{ color: "white" }}>{todaysMinimumDesktop}</span>ms
                <MobileOutlined
                  style={{
                    color: "lightgray",
                    marginLeft: "1rem",
                ...
                />{" "}
                <span style={{ color: "white" }}>{todaysMinimumMobile}</span>ms
              </div>
            </Col>
          )}
          <Col xs={24} sm={24} md={15} lg={15} xl={15}>
            <div style={{ width: "100%", height: containerHeight }} ref={ref}>
              {testStarted && !testEnded ? null : (
                <div
                  style={{
                    width: "100%",
                    position: "absolute",
              ...
              <img
                src={require("../../images/reaction/reaction-1-background.png")}
                style={styles.layers}
                alt="Valorant background"
              />
              {targetAppear ? (
                <img
                  src={require("../../images/reaction/reaction-2-target.png")}
                  style={styles.layers}
                  alt="Valorant Target Jett"
                />
              ) : null}
              <img
                src={require("../../images/reaction/reaction-3-wall.png")}
                style={styles.layers}
                alt="Covering Wall"
              />
              <img
                src={require("../../images/reaction/reaction-4-crosshair.png")}
                style={styles.layers}
                alt="Covering Corsshair"
              />
                ...
                {testEnded ? (
                  ...
                  >
                    ...
                    {invalidShots === 3 ? (
                      ...
                      >
                        <div>티어 {isMobile ? "(모바일)" : "(데스크톱)"}</div>
                        <img
                          src={`${staticServer}/icons/${getTier(
                            resultPercentage,
                          )}.png`}
                          style={{ width: 80, height: 80 }}
                          alt="reaction time rank"
                        />
                        <div>
          ...
          <Col xs={24} sm={24} md={9} lg={9} xl={9}>
                ...
                <>
                  ...
                  >
                    {testEnded ? (
                      ...
                      <img
                        src={require("../../images/reaction/fire.png")}
                        style={{ width: 50 }}
                        alt="fire a shot"
                      />
                    ) : (
            ...
            <Paragraph
              ...
              copyable={{ text: "https://valog.gg/reaction" }}
            >
              친구에게 URL 공유
            </Paragraph>
          </Col>
          ...
          >
            <div>
              본 측정은 일명 '예측샷'을 방지하기 위해 여러번의 측정을 시행하며
              최대값/최소값을 제외한 기록의 평균을 측정합니다.{" "}
              <span
                style={{ fontWeight: "bold", textDecorationLine: "underline" }}
              >
                또한 100ms 미만의 샷은 예측샷으로 간주하여 무효샷 처리가 됩니다
              </span>
              (올림픽에서 규정하는 인간의 한계 반응속도).
            </div>
            <div style={{ margin: "1rem 0" }}>
              데스크톱을 기준으로 맞추어져 있으며 모니터의 주사율, 모바일의 경우
              기기 성능에 따라 일부 차이가 날 수 있습니다. 따라서 정확한 측정을
              위해서는 데스크톱 이용을 권장드립니다(현재 누적된 데이터 통계 상
              모바일 반응속도 - 100ms = 데스크톱 반응속도의 평균을 보여주고
              있습니다. 표시되는 티어와 상위 퍼센티지는 데스크톱과 모바일이
              분리되어 계산됩니다).{" "}
            </div>
            {isMobile ? (
              <div style={{ margin: "1rem 0" }}>
                모바일의 경우 발사 버튼을 '터치'하는 순간 측정하므로 스크롤할때
                발사 버튼을 터치하지않도록 주의해주세요!
              </div>
            ) : null}
            <div style={{ fontWeight: "bold", margin: "1rem 0" }}>
              타 반응속도 측정 사이트와 차이가 존재 하는 이유?{" "}
            </div>
            <div>
              빨간 전면 배경색이 초록 전면 배경색으로 바뀌는 것과 같이 크나큰
              대조를 주는 것이 아니라 본 시뮬레이터는 비교적 작고 좁은 편의
              시각적 차이를 얼마나 빨리 인식할 수 있는지를 테스트 하는것이기
              때문에 어느정도 차이가 존재할 수 있습니다.
            </div>
          </div>
        </Row>
      </Col>
