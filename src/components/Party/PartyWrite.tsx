...
export default function PartyWrite(props) {
  ...
  const handlePreferredAgent = useCallback(
    (agentName, checked) => {
      const next = checked
        ...
        : preferredAgent.filter((t) => t !== agentName)
  ...
  const submitWrite = useCallback(() => {
    if (
      ...
      let filteringString =
        writeDescription + " " + writeUsername + " " + writeUsertag //연결해서 셋 다 검사
      if (badWords.some((v) => filteringString.includes(v))) {
        window.location.reload()
      } else {
        // console.log('금지어 없음')
        axios
          .post(`${apiServer}/party/write`, {
            data: {
              mode: writeMode,
              tier: writeTier,
    ...
    >
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Input.Password
          ...
          style={{ width: 150 }}
        />
        <div style={{ color: "gray", margin: "1rem 0" }}>
          비밀번호는 암호화되어 저장되며, 파티 모집 갱신/삭제 시 이용됩니다.
          영어와 숫자 이용을 권장드립니다.
        </div>
        타인을 사칭하거나 악의적인 도배는 법률에 의해 제재를 받을 수 있으며 해당
        ip는 수집되어 웹사이트 이용에 제한이 있을 수 있습니다.
      </Col>
      ...
      >
        <Button type="primary" onClick={submitWrite} style={{ width: "40%" }}>
          {" "}
          작성{" "}
        </Button>
      </div>
