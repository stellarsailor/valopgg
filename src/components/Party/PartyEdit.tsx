...
export default function PartyEdit(props) {
  ...
  const [writeCurrentMember, setWriteCurrentMember] = useState<any>(
    currentMember,
  )
  ...
  const [writePreferredAgent, setWritePreferredAgent] = useState<any>(
    preferredAgent.split(","),
  )
  const handlePreferredAgent = useCallback(
    (agentName, checked) => {
      const next = checked
        ? [...writePreferredAgent, agentName]
        : writePreferredAgent.filter((t) => t !== agentName)
    ...
    if (
      ...
      message.error(
        <span style={{ color: "black" }}>
          빠뜨린 부분은 없는지 다시 확인해주세요.
        </span>,
      )
    } else {
      axios
        .put(`${apiServer}/party/update/${id}`, {
          data: {
            mode: writeMode,
            tier: writeTier,
        ...
        .then((res) => {
          ...
          if (res.data.affectedRows === 1) {
            ...
            message.error(
              <span style={{ color: "black" }}>
                빠뜨린 부분은 없는지, 비밀번호가 일치하는지 확인해주세요.
              </span>,
            )
          }
        })
        .catch(function (error) {
          message.error(
            <span style={{ color: "black" }}>잠시후 다시 시도해주세요.</span>,
          )
        })
    }
  }, [
  ...
  const deleteParty = useCallback(() => {
    axios
      .post(`${apiServer}/party/delete/${id}`, {
        data: {
          password: writePassword,
        },
      })
  ...
  return (
    ...
    >
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Select
          defaultValue={writeMode}
          style={{ width: 120, color: "black" }}
          onChange={(v) => setWriteMode(v)}
        >
          {modeArray.map((v, index) => (
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Select
          defaultValue={tier}
          style={{ width: 120, color: "black" }}
          onChange={(v) => setWriteTier(v)}
        >
          {tierArray.map((v, index) => (
            <Option key={v} value={index}>
              {v}
            </Option>
          ))}
      ...
      <Col span={24} style={eachColumn}>
        ...
        {currentMemberArray.map((v) => {
          if (v <= writeCurrentMember) {
            return (
              <CursorSpan key={Math.random()} onClick={() => setWriteCurrentMember(v)}>
                <UserOutlined
                  style={{ color: "gold", fontSize: "1.4rem", margin: 5 }}
                />
              </CursorSpan>
            )
          } else {
            return (
              <CursorSpan key={Math.random()} onClick={() => setWriteCurrentMember(v)}>
                <UserOutlined
                  style={{ color: "gray", fontSize: "1.4rem", margin: 5 }}
                />
              </CursorSpan>
            )
          }
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Checkbox
          checked={writeMicNeed}
          onChange={(e) => setWriteMicNeed(e.target.checked)}
          style={{ color: "white" }}
        >
          {writeMicNeed ? (
            <span>
              <AudioOutlined style={{ color: "#ff324c" }} /> 필수
            </span>
          ) : (
            <span>
              <AudioMutedOutlined /> 자유
            </span>
          )}
        </Checkbox>
      </Col>
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Input
          ...
          onChange={(v) => setWritePlaytime(v.target.value)}
        />{" "}
        시간
      </Col>
        ...
        <div style={{ margin: 10, width: "100%" }}>
          {agent.map((v) => (
            <CheckableTag
              ...
              onChange={(checked) =>
                handlePreferredAgent(v.name.toLowerCase(), checked)
              }
            ...
            >
              <img
                ...
                style={{
                  width: "2rem",
                  borderRadius: "0rem",
                  border: `0px solid ${v.signature_color}`,
                }}
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Input
          onChange={(v) => setWriteUsername(v.target.value)}
          placeholder="닉네임"
          style={{ width: 150 }}
          value={writeUsername}
        />
        <span
          style={{ fontWeight: "bold", fontSize: "1.4rem", margin: "0 0.5rem" }}
        >
          #
        </span>
        <Input
          onChange={(v) => setWriteUsertag(v.target.value)}
          placeholder="태그"
          style={{ width: 100 }}
          value={writeUsertag}
        />
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Input.TextArea
          onChange={(v) => setWriteDescription(v.target.value)}
      ...
      <Col span={24} style={eachColumn}>
        ...
        <Input.Password
          onChange={(v) => setWritePassword(v.target.value)}
          placeholder="비밀번호"
          maxLength={16}
          style={{ width: 150 }}
        />
      ...
      >
        <Button type="primary" onClick={submitEdit} style={{ width: "40%" }}>
          {" "}
          수정/갱신{" "}
        </Button>
        <Button
