...
export default function PartyRenderer(props) {
  ...
  const selectedAgent =
    preferredAgent === null ? [""] : preferredAgent.split(",")
        ...
        <div style={{ backgroundColor: "rgb(24, 35, 56)", padding: "1rem" }}>
          ...
          <div style={{ minHeight: 70 }}>
            {selectedAgent[0] === "" ? (
              ...
              selectedAgent.map((v) => (
                <img
                  key={Math.random()}
                  src={`${staticServer}/agents/${v}-headshot.png`}
                  style={{
                    width: "2rem",
                    borderRadius: "2rem",
                    border: `2px solid ${
                      agent.filter(
                        (original) => original.name.toLowerCase() === v,
                      )[0].signature_color
                    }`,
                    margin: 5,
                  }}
                  alt="agent's portrait"
                />
              ))
            )}
          </div>
          ...
          >
            <div style={{ color: "gray" }}>
              예상 플레이 시간 :{" "}
              <span style={{ color: "white" }}>{playtime}시간</span>
            </div>
