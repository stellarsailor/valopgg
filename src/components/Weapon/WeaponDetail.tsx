...
export default function WeaponDetail(props) {
  ...
  const [
    selectedSkinElement,
    setSelectedSkinElement,
  ] = useState<skinType | null>(null)
  ...
  const selectedWeapon = weapon.filter(
    (v) => v.name.toLowerCase() === weaponName,
  )[0]

  useEffect(() => {
    setLoading(true)
    Axios.get(`${apiServer}/weapon/skins/${selectedWeapon.id}`).then((res) => {
      setLoading(false)
      // console.log(res.data)
      setSkins(res.data)
      setSelectedSkinElement(
        res.data.filter((v) => v.category === initialWeaponSkin)[0],
      )
    ...
    >
      ...
      <Col xs={24} sm={22} md={20} lg={20} xl={15}>
        <MetaTags>
          ...
          <meta
            name="description"
            content={`${selectedWeapon.name_ko} 탄창, 관통력, 주 공격모드, 보조 공격모드, 연사 횟수, 데미지, 무기 스킨 등 자세한 정보를 확인하세요.`}
          />
        </MetaTags>
        <div style={{ fontSize: "1.1rem", color: "gray", margin: 10 }}>
          ...
          {window.innerWidth < 576 ? (
            ...
            <Link to="/weapon" style={{ color: "white" }}>
              {selectedWeapon.category}
            </Link>
          )}
          <span style={{ marginLeft: 10, marginRight: 10 }}>{">"}</span>
        ...
        <Row style={{ backgroundColor: "#202b43" }}>
          {window.innerWidth < 576 ? null : (
            ...
            >
              ...
              {weapon
                .filter((v) => v.category === selectedWeapon.category)
                .map((v, index) => (
                  <WeaponRender
                    key={index}
                    name={v.name}
                    ...
                    cost={v.cost}
                    resize={
                      selectedWeapon.category === "Sidearm" ? true : false
                    }
          ...
          <Col xs={24} sm={16} md={16} lg={20} xl={20}>
            ...
            {!loading && selectedSkinElement && (
              <Row
                style={{ margin: 10, backgroundColor: "rgb(24, 35, 56)" }}
                justify="center"
              >
                <Col
                  xs={24}
                ...
                >
                    ...
                    {({ zoomIn, zoomOut, resetTransform, ...rest }) => (
                      ...
                      >
                        ...
                        <TransformComponent>
                          ...
                          >
                            <img
                              src={`${staticServer}/weaponskins/${
                                selectedSkinElement.category
                              }-${selectedWeapon.name.toLowerCase()}${
                                selectedSkinElement.level === 0
                                  ? ""
                                  : `-${selectedSkinElement.level}`
                              }.png`}
                              style={{ width: "90%" }}
                              alt="발로란트 무기 스킨"
                            />
                  ...
                  {selectedSkinElement.level !== 0 ? (
                    <div style={{ textAlign: "center", marginTop: 10 }}>
                      {skins
                        .filter(
                          (v) => v.category === selectedSkinElement.category,
                        )
                        .map((bundle) => (
                          <CursorSpan
                            key={bundle.level}
                            ...
                            onClick={() => {
                              setSelectedSkinElement(
                                skins.filter((f) => f.id === bundle.id)[0],
                              )
                              weaponScroll()
                            }}
                          >
                  ...
                  <SkinText>
                    ...
                    >
                      {selectedSkinElement.cost}{" "}
                      {parseInt(selectedSkinElement.cost) > 0
                        ? ` (= 약 ${parseInt(selectedSkinElement.cost) * 11}원)`
                        : null}
                    </span>
                  </SkinText>
                  <Col span={24}>
                            ...
                            >
                              <CursorSpan
                                style={{
                                  ...
                                  padding: "1rem",
                                  border: `1px solid ${
                                    v.category === selectedSkinElement.category
                                      ? "#00ffae"
                                      : "gray"
                                  }`,
                                }}
                ...
                >
                  <Row>
                    <SpecTitle>상세 정보</SpecTitle>
                    <SpecLeft>탄창</SpecLeft>
                    <SpecRight>{selectedWeapon.capacity}</SpecRight>
                    <SpecLeft>장착 시간</SpecLeft>
                    <SpecRight>{selectedWeapon.equipTime} 초</SpecRight>
                    <SpecLeft>재장전 시간</SpecLeft>
                    <SpecRight>{selectedWeapon.reloadTime} 초</SpecRight>
                    <SpecLeft>관통력</SpecLeft>
                    <SpecRight>{selectedWeapon.penetration}</SpecRight>
                    <SpecTitle>주 공격</SpecTitle>
                    <SpecLeft>모드</SpecLeft>
                    <SpecRight>{selectedWeapon.primaryMode}</SpecRight>
                    <SpecLeft>연사 횟수</SpecLeft>
                    <SpecRight>{selectedWeapon.primaryRate}</SpecRight>
                    {selectedWeapon.altMode ? (
                      <>
                        <SpecTitle>보조 공격</SpecTitle>
                        <SpecLeft>모드</SpecLeft>
                        <SpecRight>{selectedWeapon.altMode}</SpecRight>
                        <SpecLeft>연사 횟수</SpecLeft>
                        <SpecRight>{selectedWeapon.altRate}</SpecRight>
                      </>
                    ) : null}
                    ...
                    {selectedWeapon.damage[0] !== undefined ? (
                      <Col xs={8} sm={8} md={8} lg={8} xl={8}>
                        ...
                        <Row justify="center">
                          ...
                          >
                            <img
                              src={require(`../../images/body.png`)}
                              style={{ width: "2rem" }}
                              alt="발로란트 거리별 무기 데미지"
                            />
                          </Col>
                          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                            {selectedWeapon.damage[0].damageSpec.map(
                              (v, index) => (
                                <div
                                  style={{
                                    fontWeight: "bold",
                    ...
                    {selectedWeapon.damage[1] !== undefined ? (
                      <Col xs={8} sm={8} md={8} lg={8} xl={8}>
                        ...
                        <Row justify="center">
                          ...
                          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                            {selectedWeapon.damage[1].damageSpec.map(
                              (v, index) => (
                                <div
                                  style={{
                                    fontWeight: "bold",
                    ...
                    {selectedWeapon.damage[2] !== undefined ? (
                      <Col xs={8} sm={8} md={8} lg={8} xl={8}>
                        ...
                        <Row justify="center">
                          ...
                          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                            {selectedWeapon.damage[2].damageSpec.map(
                              (v, index) => (
                                <div
                                  style={{
                                    fontWeight: "bold",
